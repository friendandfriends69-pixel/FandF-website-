...
Colors and logo mark are pulled directly from the official Friend&Friends
business card:

- Navy `#1B2144` — primary dark / header / footer
- Teal `#0B96A7` — primary accent
- White / light grey — backgrounds and text-on-dark

The logo icon (`assets/logo-icon.png`) is the actual person+boxes mark from
the business card, used in the header and footer next to the "Friend&Friends"
wordmark.
...
- **Colors / type / spacing** — all design tokens are CSS custom properties
  at the top of `css/style.css` under `:root`. Note: the `--amber` /
  `--amber-dark` variable names are legacy from an earlier palette but now
  hold the brand teal values — rename them if you'd like, functionality is
  unaffected.
- **Logo** — replace `assets/logo-icon.png` with a higher-resolution export
  of the mark if you have the original vector/AI file, then keep the same
  filename or update the `<img src="assets/logo-icon.png">` references in
  `index.html`.
